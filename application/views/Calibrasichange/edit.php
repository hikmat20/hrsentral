...
<form action="#" method="POST" id="form_proses_bro">   
	<div class="box box-primary">
		...
		<div class="box-body">
			 ...
			 <label class='label-control col-sm-2'><b>Colomn Change<span class='text-red'>*</span></b></label>
					<div class='col-sm-4'>
						<div class='input-group'>
							<span class='input-group-addon'><i class='fa fa-user'></i></span> 
								<?php
							$contract_id[0]	= 'Select An Option';						
							echo form_dropdown('change',$contract_id,'0', array('id'=>'change','class'=>'form-control input-sm'));											
						?>
						</div>
				</div>
			</div>
			<div class='form-group row'>
			  <label class='label-control col-sm-2'><b>Contract Name<span class='text-red'>*</span></b></label>
				<div class='col-sm-4'>
					<div class="input-group">
						<span class="input-group-addon"><i class="fa fa-file"></i></span>              
						<?php
							$contract_id[0]	= 'Select An Option';						
							echo form_dropdown('contract_id',$contract_id,'0', array('id'=>'contract_id','class'=>'form-control input-sm'));											
						?>
					</div>
				</div>
			</div>
			...
			<div class='form-group row'>			
				<label class='label-control col-sm-2'><b>Latest Date <span class='text-red'>*</span></b></label>
				<div class='col-sm-4'>
					<div class="input-group">
						<span class="input-group-addon"><i class="fa fa-calendar"></i></span>              
						<?php
							echo form_input(array('id'=>'latestcontract_date','name'=>'latestcontract_date','class'=>'form-control input-sm','autocomplete'=>'off','placeholder'=>'Latest Date'));											
						?>
					</div>	
				</div>
			</div>
...
<script>
	$(document).ready(function(){
		...
		$('#change').change(function(){
			...
			$("#flag_active").fadeIn(500);
			}
			else
			{
			$("#flag_active").fadeOut(500);
			}
		});
		...
		$('#simpan-com').click(function(e){
			...
			var start	= $('#startcontract_date').val();
			var latest	= $('#latestcontract_date').val();
			
			if(change!= contract){
				swal({
				  title	: "Error Message!",
				  text	: 'Colomn Change Value Must Equal With Contract Name.....',
				  type	: "warning"
				});
			...
			if(change=='CTR004'){
				...
				swal({
				  title	: "Error Message!",
				  text	: 'Start Date Value Must Equal With Latest Date.....',
				  type	: "warning"
				});
			...
			if(contract=='' || contract==null || contract=='0'){
				swal({
				  title	: "Error Message!",
			...
			if(latest=='' || latest==null || latest=='0'){
				swal({
				  title	: "Error Message!",
				  text	: 'Empty Latest Date, please input Latest Date first.....',
				  type	: "warning"
				});
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=base_url + active_controller +'/ubahstatus';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
