    ...
    <div class="box-body">
        <form class="form-horizontal" id="form-substituion" enctype="multipart/form-data">
            <div class="row">
                <div class="col-md-6">
                    <div class="hidden">
                        <input type="text" class="form-control" name="flag_revision" id="flag_revision" readonly value="<?= $wfh->flag_revision; ?>">
                        <input type="text" class="form-control" name="no_revision" id="no_revision" readonly value="<?= $wfh->no_revision; ?>">

                        <div class="form-group">
                            <label for="employee_id" class="col-sm-3 control-label">Employee <span class="text-red">*</span></label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" name="id" id="id" readonly value="<?= $wfh->id; ?>">
                                <input type="text" class="form-control" name="employee_id" id="employee_id" readonly value="<?= $wfh->employee_id; ?>">
                                <input type="text" class="form-control" name="name" id="employee_name" readonly value="<?= $wfh->name; ?>">
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" name="division_id" readonly value="<?= $wfh->division_id; ?>">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="periode_year" class="col-sm-3 control-label">Year Periode <span class="text-red">*</span></label>
                            <div class="col-sm-9">
                                <input type="text" name="periode_year" class="form-control" value="<?= ($wfh->periode_year) ? $wfh->periode_year : date('Y'); ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <div class="input-group">
                                ...
                                <input type="date" name="from_date" min="<?= date('Y-m-d', strtotime(date('Y-m-d') . "-1 Month")); ?>" value="<?= $wfh->from_date; ?>" class="form-control" id="from_date">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <div class="input-group">
                                <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                                <input type="date" name="until_date" min="<?= date('Y-m-d', strtotime(date('Y-m-d') . "-1 Month")); ?>" value="<?= $wfh->until_date; ?>" class="form-control" id="until_date">
                            </div>
                        </div>
                    </div>

                    <div class="form-group" id="input-type-leave" style="display: <?= ($wfh->type_day == 'HALF_DAY') ? '' : 'none'; ?>;">
                        ...
                        <div class="col-sm-9">
                            <select name="type_day" id="type_day" class="form-control">
                                <option value=""></option>
                                <option value="HALF_DAY" <?= ($wfh->type_day == 'HALF_DAY') ? 'selected' : ''; ?>>Half Day</option>
                                <option value="FULL_DAY" <?= ($wfh->type_day == 'FULL_DAY') ? 'seleted' : ''; ?>>Full Day</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <div class="input-group">
                                <input type="text" readonly name="total_days" value="<?= $wfh->total_days; ?>" class="form-control" id="total_days" placeholder="0">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            <textarea name="reason" id="reason" class="form-control" rows="3" required="required" placeholder="Alasan"><?= $wfh->reason; ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-group hidden">
                        ...
                        <div class="col-md-9">
                            <input type="text" name="approval_by" id="approval_by" value="<?= ($wfh->approval_by) ? $wfh->approval_by : '' ?>">
                        </div>
                    </div>
                </div>
            </div>
    ...
    $(document).on('click', '#save', function() {
        ...
        if (from_date == '') {
            swal({
                title: 'Perhatian!',
                type: 'warning',
                text: 'Tanggal awal pengajuan WFH belum di isi.'
            })
            return false;
        } else if (until_date == '') {
            swal({
                title: 'Perhatian!',
                type: 'warning',
                text: 'Tanggal akhir pengajuan WFH belum di isi.'
            })
            return false;
            ...
            swal({
                title: 'Perhatian!',
                type: 'warning',
                text: 'Alasan pengajuan WFH belum di isi.'
            })
            return false;
        } else if (approval == '') {
            swal({
        ...
        $.ajax({
            ...
            success: function(result, response) {
                if (result.status == 1) {
                    ...
                    setTimeout(function() {
                        window.open(base_url + active_controller);
                    }, 1500)
