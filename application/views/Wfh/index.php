    ...
    <div class="box-body">
        <div class="text-lg-right" style="margin-bottom: 10px; display:block">
            <?php if ($access['create'] == '1') : ?>
                <a href="<?= base_url(); ?>wfh/add" class=' btn btn-md btn-primary' title='Create Application' data-role='qtip'><i class='fa fa-plus'></i> New Applications</a>
            <?php endif; ?>
        </div>
                                ...
                                <td>
                                    <?php if ($ses_userId == $data->approval_employee_id) : ?>
                                        <?php if ($access['approve'] == '1') : ?>
                                            <a href="<?= base_url('wfh/approval/' . $data->id); ?>" data-id="<?= $data->id; ?>" id="approve" data-action="approve" class='btn btn-sm btn-success' title='Approve Application' data-role='qtip'><i class='fa fa-check'></i></a>
                                        <?php endif ?>
                                    <?php else : ?>
                                        <?php if ($data->status == 'OPN') : ?>
                                            <?php if ($access['read'] == '1') : ?>
                                                <a href="<?= base_url(); ?>wfh/view/<?= $data->id; ?>" class=' btn btn-sm btn-primary' title='View Application' data-role='qtip'><i class='fa fa-eye'></i></a>
                                            <?php endif ?>
                                            <?php if ($access['update'] == '1') : ?>
                                                <a href="<?= base_url(); ?>wfh/edit/<?= $data->id; ?>" class=' btn btn-sm btn-warning' title='Edit Application' data-role='qtip'><i class='fa fa-pencil'></i></a>
                                            <?php endif ?>
                                            ...
                                            <?php endif ?>
                                            <?php if ($access['create'] == '1') :
                                                $text = "Dengan Hormat,%0aSaya yang bertanda tangan dibawah ini :%0a%0aNama : " . $data->name . "%0aDivisi : " . $data->divisions_name . "%0a%0aBermaksud untuk mengajukan Work From Home (WFH) pada tanggal " . $data->from_date . " s/d " . $data->until_date . " selama " . $data->total_days . " hari.%0a%0aUntuk lebih detailnya bisa klik link dibawah ini:%0a" . base_url('wfh/view/' . $data->id) . "%0a%0aDemikian surat izin work from home ini saya sampaikan. Atas perhatiannya saya ucapkan terima kasih.%0a%0aHormat Saya,%0a" . $data->name;
                                            ?>
                                        ...
                                        <?php elseif ($data->status == 'REV') : ?>
                                            <?php if ($access['update'] == '1') : ?>
                                                <a href="<?= base_url(); ?>wfh/update_revision/<?= $data->id; ?>" class=' btn btn-sm btn-warning' title='Update Application' data-role='qtip'><i class='fa fa-pencil'></i></a>
                                            <?php endif ?>
                                            <?php if ($access['read'] == '1') : ?>
                                        ...
                                        <?php else : ?>
                                            ...
                                            <?php if ($this->session->userdata('Group')['id'] == 40) : ?>
                                                <a href="<?= base_url(); ?>wfh/approval/<?= $data->id; ?>" data-action="view" class=' btn btn-sm btn-success' title='Approval Application' data-role='qtip'><i class='fa fa-check'></i></a>
                                            <?php endif; ?>
                                        <?php endif ?>
        ...
        $(document).on('click', '#sendEmail', function() {
            ...
            swal({
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        loading_spinner();
                        var baseurl = '<?= base_url(); ?>' + 'wfh/sendEmail';
                        $.ajax({
                            url: baseurl,
                            type: "POST",
                            data: {
