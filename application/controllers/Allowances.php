...
class Allowances extends CI_Controller {
	...
	public function index(){
		...
		$get_Data			= $this->employees_model->getDataAllowance();
		...
		history('View Data Allowances');
		$this->load->view('Allowances/index',$data);
	}
	...
	public function add($id=''){
		...
		if($this->input->post()){
			...
			$data['id']				= $this->employees_model->code_otomatis('ms_allowance','ALL');
			$data['created_by']		= $data_session['User']['username']; 
			$data['created']		= date('Y-m-d H:i:s');
			
			if($this->employees_model->simpan('ms_allowance',$data)){
			...
			$this->load->view('Allowances/add',$data);
		}
	}
	...
	public function edit($id=''){
		if($this->input->post()){
			...
			if($this->master_model->getUpdate('ms_allowance',$data,'id',$this->input->post('id'))){
				$Arr_Kembali		= array(
					'status'		=> 1,
			...
			$get_Data5			= $this->employees_model->getPositions($arr_Where);
			$detail				= $this->master_model->getData('ms_allowance','id',$id); 
			$data = array(
			...
			$this->load->view('Allowances/edit',$data);
		}
	}
