...
class Bpjs extends CI_Controller {
	...
	public function index(){
		...
		$get_Data			= $this->employees_model->getDataBpjs();
		$Allowances			= $this->employees_model->getEmployees();
		$data = array(
			'title'			=> 'Indeks Of BPJS',
			'action'		=> 'index',
			'religi'		=> '0',
			'row'			=> $get_Data,
			'data_menu'		=> $Allowances,
			'akses_menu'	=> $Arr_Akses
		);
		history('View Data BPJS');
	...
	public function add($id=''){
		...
		if($this->input->post()){
			...
			$data['id']				= $this->employees_model->code_otomatis('ms_bpjs','BPJS');
			$data['created_by']		= $data_session['User']['username']; 
			$data['created']		= date('Y-m-d H:i:s');
			
			if($this->employees_model->simpan('ms_bpjs',$data)){
				
				$Arr_Kembali		= array(
					'status'		=> 1,
					'pesan'			=> 'Add BPJS Success. Thank you & have a nice day.......'
				);
				history('Add Data Master BPJS'.$data['id']);
			}else{
				$Arr_Kembali		= array(
					'status'		=> 2,
					'pesan'			=> 'Add BPJS failed. Please try again later......'
				);
	...
	public function edit($id=''){
		if($this->input->post()){
			...
			if($this->master_model->getUpdate('ms_bpjs',$data,'id',$this->input->post('id'))){
				$Arr_Kembali		= array(
					'status'		=> 1,
	...
	function delete($id){
		...
		if($this->db->affected_rows()>0){
			...
			history('Delete Data Allowances id'.$id);
			redirect(site_url('Allowances'));
		}
	}
	...
	public function load_detail()
    {
        ...
        $numb = 1;
        $data = $this->employees_model->getData('ms_bpjs'); 
